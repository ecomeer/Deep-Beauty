...
// Sandboxed dev environments ship a local Chromium and route HTTPS through a
// TLS-intercepting proxy the browser must use and trust. CI never takes this
// branch — it installs and runs its own pinned Chromium directly.
if (!process.env.CI && existsSync(LOCAL_CHROMIUM)) {
  use.launchOptions = { executablePath: LOCAL_CHROMIUM }
  if (process.env.HTTPS_PROXY) {
    use.proxy = { server: process.env.HTTPS_PROXY, bypass: 'localhost,127.0.0.1' }
