...
export default function LoginClient() {
  ...
  const handleMagicLinkSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        toast.error(translateAuthError(error, 'حدث خطأ أثناء إرسال رابط الدخول'))
      } else {
        setMagicSent(true)
      }
    } catch {
      toast.error('حدث خطأ أثناء إرسال رابط الدخول')
    } finally {
      setLoading(false)
    }
  }
    ...
    >
      ...
      >
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden" style={{ border: '1px solid var(--beige)' }}>
          ...
          <div className="flex gap-1 p-1.5 mx-6 mt-6 bg-gray-100 rounded-xl">
            <button
              type="button"
          ...
          {mode === 'magic' ? (
            magicSent ? (
              <div className="p-6 text-center">
                ...
                <h2 className="font-bold mb-1">تحققي من بريدك الإلكتروني</h2>
                <p className="text-sm text-gray-500">
                  أرسلنا رابط دخول إلى <span dir="ltr">{magicEmail}</span> — افحصي بريدك (ومجلد السبام) واضغطي الرابط لتسجيل الدخول
                </p>
              </div>
            ) : (
              <form onSubmit={handleMagicLinkSubmit} className="p-6 space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-2">سنرسل لك رابطاً لتسجيل الدخول بدون كلمة مرور — ينشئ حساباً تلقائياً إن لم يكن لديكِ واحد</p>
                </div>
                <motion.button
